...
| `x/<数量><格式><大小> <地址>` | 查看内存（e.g. `x/4xw 0x20000000`） |
...
`数量`:一个数字,表示要查看多少单位的数据,默认为1

`格式`:

| 代码 | 含义                              | 示例                                              |
| :--- | :-------------------------------- | :------------------------------------------------ |
| `s`  | 字符串 (String)                   | `x/s $x1` 显示地址 `$x1` 处的字符串               |
| `i`  | 指令 (Instruction)                | `x/i $pc` 显示当前程序计数器 (`$pc`) 处的汇编指令 |
| `d`  | 十进制数 (Decimal)                | `x/d $sp` 显示栈顶地址处的 4 字节十进制整数       |
| `u`  | 无符号十进制数 (Unsigned Decimal) |                                                   |
| `x`  | 十六进制数 (Hexadecimal)          | `x/x $x0` 显示寄存器 `$x0` 处的十六进制值         |
| `o`  | 八进制数 (Octal)                  |                                                   |
| `t`  | 二进制数 (Binary)                 | `x/t $x0` 显示二进制值                            |
| `a`  | 地址 (Address)                    | `x/a 0x1000` 显示地址，并尝试解析为符号名         |
| `c`  | 字符 (Char)                       |                                                   |

`大小`:每次读取多少,相当于前面数量的单位
...
> 消息队列属于 Linux 进程间通信（IPC, Inter-Process Communication）的重要机制之一，介于“管道（pipe）”与“共享内存（shm）”之间。
>
> 它是一种内核维护的消息链表，每条消息都带有一个类型和一段数据，多个进程可以通过系统调用往队列中写入或读取消息。

#### key_t与ftok()

> 了解key_t和ftok的工作机理,有利于我们使用后面的各种IPC进行通信

**key_t的由来**

System V IPC（Inter-Process Communication）机制诞生于 1980 年代早期，当时设计者要让多个进程在**不共享文件描述符**的情况下通信。

一个思路是:

> 在内核中建立一张“IPC 对象表”，每个对象（如消息队列、信号量集、共享内存段）都有一个唯一标识符 `key_t key`，类似一个“名字”。

不同进程只有持有同一个key,就能访问同一IPC对象

key在定义是是一个整数(`typedef int key_t`),为了让不同进程能够使用同一个key_t,Linux提供了一个生成规则:

`ftok()`[即`filetokey`]的简写
...
ftok函数根据一个已存在的文件路径和一个项目ID(单字节整数),生成一个**系统唯一**的key
...
| 部分        | 含义                                                  |
| ----------- | ----------------------------------------------------- |
| `st.st_dev` | 文件所在设备号（区分不同磁盘/分区）                   |
| `st.st_ino` | 文件的 inode 号（区分同一设备上的文件）               |
| `proj_id`   | 用户自定义的 8 bit 项目 ID，用于人为区分不同 IPC 对象 |

这样生成的 key 只要 (pathname, proj_id) 组合不同，一般就能保证唯一。

**使用规则和注意事项**

1. 文件必须存在

   ftok()内部会调用stat()读取文件信息;如果路径不存在,会返回(key_t) -1

2. `proj_id`通常用一个字母来表示

   比如ftok(".",'A'),A在ASCII中就是一个整数值,便于区分

3. 不保证绝对唯一

   如果两个文件在同一设备上的inode号相同(例如tmpfs环境),或者proj_id相同

   可能会冲突,尽管在普通文件系统中概率极低

4. 所有System V IPC都适用ftok生成的key

   ​	消息队列 → `msgget(key, flag)`

   ​	共享内存 → `shmget(key, flag)`

   ​	信号量 → `semget(key, flag)`

   多个进程只要使用相同的 `(pathname, proj_id)` 组合，就会得到同一个 key，从而访问同一个 IPC 对象。



#### System V消息队列

System V 消息队列在 Linux 内核中是通过一个叫 `msg_queue` 的结构体维护的（位于 `ipc/msg.c` 内核源文件中）。
 当我们创建一个队列时，内核会分配一块内核内存来保存这个结构体，其中包括：
...
> 换句话说：**System V 消息队列是一种驻留在内核态的、带类型的链表。**

**使用流程**

我们从程序编写角度看，它的使用一般分为五个步骤：

1. **创建或打开消息队列：** `msgget()`
2. **发送消息：** `msgsnd()`
3. **接收消息：** `msgrcv()`
4. **控制操作（删除/查看属性）：** `msgctl()`
5. **关闭程序时释放资源（内核自动删除或手动删除）**
...
struct msgbuf {
    ...
    char mtext[128]; // 消息内容（自定义长度）在后面的函数中消息长度不包括long mtype的长度
};
```

`mtype` 是非常关键的：
 当你接收消息时，可以根据类型来选择性接收：

- `msgrcv(..., msgtyp = 1, ...)` 表示只接收类型为 1 的消息；
- `msgtyp = 0` 表示接收队列中第一条消息；
- `msgtyp < 0` 表示接收类型 ≤ |msgtyp| 的最小类型的消息。

这使得不同功能模块的进程能共享一个队列，却按类型区分消息。
...
内核会查找是否存在该key对应的队列
存在:返回该队列的ID
不存在且设置了IRC_CREAT:分配内核内存并注册,如果未设置返回错误
Args:
	key:队列的名字,通常通过ftok("filename", proj_id) 生成一个唯一键值。
	msgflg:权限标志(如 0666 | IPC_CREAT)
Return:
	返回值是队列 ID（非负整数），失败返回 -1。
*/
```
...
Args
	msqid:队列ID
	msgp:指向消息结构体(含mtype和mtext)
	msgsz:消息正文长度
	msgflg:0:阻塞发送,IPC_NOWAIT:非阻塞发送
*/
```
...
Args:
	msqid:消息队列ID
	msgp:要用来存储接收到消息的结构体
	msgsz:msgp中消息的大小(既sizeof(*msgp) – sizeof(long))
	msgtyp:要接受消息的种类
	msgflg:
		0:阻塞
		IPC_NOWAIT:非阻塞(没有符合条件的消息立即返回错误码ENOMSG)
		MSG_NOERROR:消息太长则截断
...
#### System V消息队列的内核阻塞和唤醒机制

> 了解**System V 消息队列的内核阻塞与唤醒机制**是理解Linux IPC内核调度思想的重要一步
>
> 也能帮助以后理解socket甚至epoll等机制
...
接收消息时,如果队列中暂时没有符合条件的消息且设置了IPC_NOWAIT就会陷入阻塞等待



**这个阻塞在内核中是如何实现的?**

Linux内核中,每个消息队列对象(struct msg_queue)内部维护两个等待队列:

```c
wait_queue_head_t q_senders; // 等待发送的进程队列（当队列满时）
wait_queue_head_t q_receivers; // 等待接收的进程队列（当队列空时）
```

当 `msgrcv()` 没有找到合适消息时，内核就执行：

```c
if (队列中无消息) {
    // 创建一个等待节点
    struct msg_receiver receiver;
    receiver.task = current; // 当前进程
    receiver.msgtyp = msgtyp;
    receiver.mode = 阻塞模式;

    // 将该接收者加入等待队列
    list_add_tail(&receiver.r_list, &msq->q_receivers);
...
> 内核通过 `schedule()` 主动挂起当前进程，
>  把 CPU 让给其他进程运行。
>  当前进程就进入“睡眠态”（阻塞态）。

从此刻开始，内核认为该进程“在等待队列 q_receivers 上睡眠”。

当另一个进程调用 `msgsnd()` 成功发送了一条消息时，
 `do_msgsnd()` 内核函数会检查队列：

```c
if (q_receivers 上有等待者 && 新消息类型匹配) {
    wake_up_process(receiver->task);//唤醒相应的接收者;
}
```

这条调用会把等待进程的状态从 `TASK_INTERRUPTIBLE` 改回 `TASK_RUNNING`，
下一次调度器分配 CPU 时，该进程就会继续从被阻塞的位置恢复执行。

恢复后 `msgrcv()` 复制数据到用户空间，返回消息长度。

如果是非阻塞模式

不会进入等待队列，也不会调用 `schedule()`，立即返回。
...
> 又名信号灯
>
> 是不同进程间或一个给定进程内部不同线程间的同步机制

在底层,信号量是一个整数计数器,用于协调多个进程或线程对共享资源的访问

信号量（Semaphore）是一个受保护的变量,只能通过三种操作来访问

| 操作               | 作用                               |
| ------------------ | ---------------------------------- |
| 初始化(Init)       | 一般初始化信号量为0                |
| P操作（Wait/Down） | 请求一个资源（如果资源不足则等待） |
| V操作（Signal/Up） | 释放一个资源（唤醒等待者）         |

在操作系统中,这两个操作通常是原子操作(不可中断的):
...
System V信号量是一个或多个计数信号量的集合

可同时操作集合中的多个信号量,申请多个资源时避免死锁
...
| 函数                                                      | 作用                                                         |
| --------------------------------------------------------- | ------------------------------------------------------------ |
| `key_t ftok(const char *pathname, int proj_id)`           | 生成唯一 key,用于后续信号量关联                              |
| `int semget(key_t key, int nsems, int semflg)`            | 创建或获取信号量集<br />返回信号量id(semid),失败返回0<br />**key**:ftok生成的key_t<br />**nsems**:信号量集中信号量的数目<br />**semflg**:是信号量集的访问权限,一般为`IPC_CREAT ` |
| `int semctl(int semid, int semnum, int cmd, ...)`         | 控制信号量（初始化、删除等）<br />**semnum**:要操作的信号量集合中的信号量变化<br />**cmd**:指定操作,`GETVAL`获取信号量并返回,<br />`SETVAL`设置信号量的值,此时需要用到第四个参数共用体<br />`IPC_RMID`:删除信号量 |
| `int semop(int semid, struct sembuf *sops, size_t nsops)` | 执行P/V操作<br />`sops`:描述对信号量操作的结构体<br />`nops`:要操作的信号量个数 |
...
```c
struct sembuf {
    unsigned short sem_num;  // 信号量编号（一个集合里可以有多个信号量）
    short sem_op;            // 操作值：-1表示P操作，+1表示V操作
    short sem_flg;           // 0 或 IPC_NOWAIT, SEM_UNDO是否阻塞
};
```
...
```c
union semun {                // semctl 的第四个参数类型
    int val;				//要设置信号量的值  对应SETVAL命令
    struct semid_ds *buf;   //获取设置信号量的状态(权限,最后操作时间等) IPC_STAT, IPC_SET 命令
    unsigned short *array; //批量操作信号量  GETALL, SETALL 命令
};
```
...
int main() {
    ...
    union semun arg;
    arg.val = 0;    
    semctl(semid, 0, SETVAL, arg);//赋值信号量
...
> POSIX信号量依旧是一个计数器,但与SYSTEM V相比,他的接口是面向信号量本身而不是通过ID句柄

你可以直接操作一个变量(sem_t)类型,而不是通过semid

他可以用于进程间同步,也可以用于线程间同步

POSIX提供两种信号量:

**无名信号量（unnamed）** —— 用于线程间同步（最常用）。

**有名信号量（named）** —— 用于不同进程间同步。
...
| `int sem_wait(sem_t *sem)`                                  | P操作（等待）        |
| `int sem_trywait(sem_t *sem)`                               | 非阻塞 P 操作        |
| `int sem_post(sem_t *sem)`                                  | V操作（释放）        |
| `int sem_getvalue(sem_t *sem, int *sval)`                   | 获取当前值           |
| `sem_open`, `sem_close`, `sem_unlink`                       | 有名信号量接口       |
...
void* child_func(void* arg) {
    for (int i = 0; i < 5; i++) {
        sem_wait(&sem_child);   // 等待父线程释放信号
        printf("Child thread printing %d\n", i);
        sem_post(&sem_parent);  // 通知父线程可以继续
    }
    return NULL;
}
...
int main() {
    pthread_t tid;
    sem_init(&sem_parent, 0, 1);  // 初始父线程可执行
    sem_init(&sem_child, 0, 0);   // 子线程需等待

    pthread_create(&tid, NULL, child_func, NULL);

    for (int i = 0; i < 5; i++) {
        sem_wait(&sem_parent);
        printf("Parent thread printing %d\n", i);
        sem_post(&sem_child);
    }
...
/*输出
Parent thread printing 0
Child thread printing 0
Parent thread printing 1
...
有名信号量其实就像“在 `/dev/shm` 中的共享文件”，允许跨进程同步。
